...
public class ModeloPais {
    ...
    public int EvitarDelete(Pais pais) {
        ...
        String sql = "SELECT COUNT(e.id_pais_edi), COUNT(a.id_pais_origen_aut)"
                + "FROM editorial e, autor a, pais p "
                + "WHERE e.id_pais_edi = p.id_pais AND a.id_pais_origen_aut = p.id_pais "
                + "AND e.id_pais_edi =  ?  AND a.id_pais_origen_aut = ?";

        try {
            ps = con.prepareStatement(sql);
            ps.setInt(1, pais.getId_pais());
            ps.setInt(2, pais.getId_pais());
            rs = ps.executeQuery();
