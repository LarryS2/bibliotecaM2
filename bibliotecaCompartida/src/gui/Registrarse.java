    ...
    private void btnlabelregistroMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnlabelregistroMouseClicked
        ...
        if (cedula.isEmpty() ||  nombre.isEmpty() || apellido.isEmpty() || 
            ...
            if(tipo_sangre.equalsIgnoreCase("SELECCIONAR")) {
                ...
                if (user.ValidarNombreYapellido(nombre)==false || user.ValidarNombreYapellido(apellido)==false
                        || user.ValidarCorreo(email)==false || user.ValidarTelefono(telefono)==false || user.Validar_fecha(fecha_nac)) {
                    JOptionPane.showMessageDialog(null, "HAY CAMPOS CON INFORMACIÓN NO VÁLIDA");
                } else {
                    if(user.ValidarCedula(cedula)){
                        if(Validar_no_Rep(cedula)) {
                            if (user.ValidarPassword(password)==false) {
                                ...
                                if(password.equals(password_rep)){
                                    ...
                                    user.setPrimer_nombre(nombre);
                                    user.setPrimer_apellido(apellido);
                                    user.setEmail(email);
                                    user.setTelefono(telefono);
                                    user.setFecha_nac(formatear_fecha(fecha_nac));
                                    user.setGenero(gen);
                                    user.setTipo_sangre(tipo_sangre);
